...
if(!nv_user_in_groups('1,2,3,10,12,13,17') and !$is_edit){ // kiểm quyền đang thêm
	nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name);
}
elseif(!nv_user_in_groups('1,2,3,10,12,13,17') and $is_edit){ // Kiểm quyền đang sửa
    // Nếu user không những nhóm này (quản trị(1,2,3), ban điều hành(10), marketing(12), kỹ thuật(13), thiết kế(17))
    nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name);
}
...
if ($row['id'] > 0) {
    ...
    if((nv_user_in_groups('13') and $row['useradd'] != $user_info['userid']) and !nv_user_in_groups('1,2,3,10')){// Kiểm quyền
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['progress'] = $nv_Request->get_int('progress', 'post', 0);
    ...
    try {
        ...
        if (empty($row['id'])) {
            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (customerid, workforceid, title, begintime, endtime, realtime, price, vat, url_code, content, files, useradd, addtime, status, progress, type_id) VALUES (:customerid, :workforceid, :title, :begintime, :endtime, :realtime, :price, :vat, :url_code, :content, :files, ' . $user_info['userid'] . ', ' . NV_CURRENTTIME . ', :status, :progress, :type_id)';
            $data_insert = array();
            ...
            $stmt->bindParam(':progress', $row['progress'], PDO::PARAM_INT);
...
$customer_info = array();
    $customer_info = nv_crm_customer_list();
...
foreach ($array_config['array_status'] as $key_status => $value_status) {
    $xtpl->assign('STATUS_CFG', array(
        'id' => $value_status['id'],
        'value' => $value_status['txt'],
        'selected' => $key_status == $row['status'] ? 'selected="selected"' : ''
    ));
