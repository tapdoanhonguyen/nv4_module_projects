 ...
 * @return
 */
function nv_theme_project_detail($rows, $content_comment, $array_control, $array_hangmuc)
{
    global $global_config, $module_name, $module_file, $lang_module, $module_config, $module_info, $op, $array_status, $site_mods, $array_field_config, $custom_fields, $array_config, $user_info;
	...
	if(nv_user_in_groups('1,2,3,18')){
		// Nếu là nhóm Kế toán
		$xtpl->parse('main.allow_createinvoice');
	}
	// print_r($rows);die();
	if($rows['useradd'] == $user_info['userid'] or nv_user_in_groups('1,2,3,10')){
		// Nếu là người tạo dự án hoặc nhóm điều hành viên(10) + quản trị 
		$xtpl->parse('main.allow_edit');
	}
    ...
    if(!empty($array_hangmuc)){
        foreach ($array_hangmuc as $hang_muc) {
            $xtpl->assign('HANG_MUC', $hang_muc);
			if($hang_muc['progress'] == 100){
				// print_r();die('as');
				$xtpl->parse('main.hang_muc.complete');
			}
			else{
				$xtpl->parse('main.hang_muc.incomplete');
			}
            if(!empty($hang_muc['cong_viec'])){
                foreach ($hang_muc['cong_viec'] as $cong_viec) {
                    $xtpl->assign('CONG_VIEC', $cong_viec);
                    $xtpl->parse('main.hang_muc.cong_viec.loop');
                }
                $xtpl->parse('main.hang_muc.cong_viec');
            }
            $xtpl->parse('main.hang_muc');
        }
    ...
    foreach ($array_config['array_status'] as $index => $value) {
