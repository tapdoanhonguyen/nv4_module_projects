...
function nv_sendinfo_projects($id)
{
    ...
    if (!empty($message)) {
		...
		$cc_id = explode(',',$row['workforceid']);
		

        $return = nv_email_send($subject, $message, 0, $sendto_id, $cc_id);
    }
...
function nv_projects_delete($id)
{
    ...
    $rows = $db->query('SELECT title, files, useradd FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE id=' . $id)->fetch();
    if ($rows) {
        
        if((nv_user_in_groups('12,13,17') and $rows['useradd'] != $user_info['userid']) or !nv_user_in_groups('1,2,3,10')){ // OLD
        // if(!nv_user_in_groups('1,2,3,10,12,13,17')){
            // Nếu user không những nhóm này (quản trị(1,2,3), ban điều hành(10), kỹ thuật(13), SEO(12), Thiết kế(17))
            return false;
            die();
        }
